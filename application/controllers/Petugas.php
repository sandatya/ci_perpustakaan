...
class Petugas extends CI_Controller {
    ...
    function __construct(){
        ...
        parent::__construct();
// cek session yang login, jika session status tidak sama dengan
//session petugas_login,maka halaman akan di alihkan kembali ke halaman login.
...
function peminjaman(){
    $data['peminjaman'] = $this->db->query("select * from peminjaman, buku, anggota where peminjaman.peminjaman_buku
    = buku.id and peminjaman.peminjaman_anggota=anggota.id order by peminjaman_id desc")->result();
...
function peminjaman_batalkan($id){
    ...
    // mengambil data buku pada peminjaman ber id terse $data = $this->m_data->edit_data($where,'peminjamanbut ')->row();
...
function peminjaman_laporan(){
    if(isset($_GET['tanggal_mulai'])&&isset($_GET['tanggal_sampai'])){
        ...
        peminjaman.peminjaman_anggota=anggota.id and date(peminjaman_tanggal_mulai) >= '$mulai' and date(peminjaman_tanggal_mulai) <= '$sampai' 
        ...
        and peminjaman.peminjaman_anggota=anggota.id order by peminjaman_id desc")->result();
    }
...
function peminjaman_cetak(){
    if(isset($_GET['tanggal_mulai']) && isset($_GET['tanggal_sampai'])){
        ...
        $data['peminjaman'] = $this->db->query("select * from peminjaman,buku,anggota where peminjaman.peminjaman_buku=buku.id and
        peminjaman.peminjaman_anggota=anggota.id and date(peminjaman_tanggal_mulai) >='$mulai' and date(peminjaman_tanggal_mulai) <= '$sampai' order by peminjaman_id desc")->result();
